...
def send_signal_to_go(symbol, action, price, score):
    """Python'daki sinyali Go Execution Engine'e gönderir"""
    url = "http://localhost:3000/api/v1/signals"
    
    # Bazen price Decimal tipinde gelebilir, float'a çeviriyoruz garanti olsun
    payload = {
        "symbol": symbol,
        "action": action,
        "price": float(price) if price else 0.0,
        "score": float(score) # Go tarafında işlemi onaylasın diye yüksek puan (99) gönderiyoruz
    }
    
    try:
        response = requests.post(url, json=payload, timeout=3)
        if response.status_code in [200, 201]:
            print(f"🚀 BAŞARILI: Sinyal Go Bot'a İletildi! -> {action} {symbol}")
        else:
            print(f"⚠️ DİKKAT: Go botu sinyali reddetti -> {response.text}")
    except Exception as e:
        print(f"❌ HATA: Go sunucusuna ulaşılamıyor (3000 portu açık mı?) -> {e}")
...
async def trader_binance(df, model: dict, config: dict, model_store: ModelStore):
    ...
    trade_score = df['trade_score'].iloc[-1] if 'trade_score' in df.columns else 0.0
    ...
    if signal_side == "BUY":
        ...
        # 📞 GO BOTUNU ARA!
        send_signal_to_go(symbol, "BUY", close_price, trade_score)
        
    elif signal_side == "SELL":
        print(f"<=== SELL SIGNAL: {signal}")
        # 📞 GO BOTUNU ARA!
        send_signal_to_go(symbol, "SELL", close_price, trade_score)
